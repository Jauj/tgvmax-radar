...
- 🤖 **Snapshot quotidien automatique** : une GitHub Action ([`.github/workflows/daily-snapshot.yml`](.github/workflows/daily-snapshot.yml)) relève chaque matin (déclencheur « watchdog » toutes les 30 min + garde-fou anti-doublon — le créneau exact dépend du planificateur GitHub, en général entre 7 h et 9 h 45 heure de Paris) la fenêtre complète pour chaque tronçon de [`data/watched.json`](data/watched.json), et versionne le résultat dans `data/history/` — consultable par tous, même sans visiter le site.
